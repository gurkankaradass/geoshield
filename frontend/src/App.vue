  ...
  <div class="flex h-screen w-screen overflow-hidden bg-gray-950 font-sans antialiased text-gray-100">
    ...
    :class="isSidebarOpen ? 'w-80 translate-x-0' : 'w-0 -translate-x-full md:w-16 md:translate-x-0'">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4" v-if="isSidebarOpen">
        ...
        <div class="bg-gray-800/60 border border-gray-700 rounded-xl p-4" v-if="!analysisResult && !isLoading">
          ...
          <div class="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
            ...
            <div class="text-xs space-y-2 border-b border-gray-700/50 pb-3">
              ...
              <p>
                ...
                <span class="font-bold text-emerald-400 text-sm">{{ analysisResult.distance_km }} km</span>
              </p>
            </div>

            <div class="space-y-2 pt-1">
              <label class="text-[11px] uppercase tracking-wider text-gray-400 font-semibold block">Bu Konumu Kaydet</label>
              <input 
        ...
        <div class="space-y-2 shrink-0">
          <h3 class="text-xs font-bold uppercase tracking-widest text-gray-500 px-1 flex items-center justify-between">
            <span>📋 Kayıtlı Mülklerim</span>
            <span class="text-[10px] bg-gray-800 text-gray-400 px-1.5 py-0.5 rounded-md font-mono">{{ savedLocations?.length || 0 }}</span>
          </h3>
...
<div class="space-y-2 max-h-[40vh] overflow-y-auto pr-1 select-none">
          ...
          class="bg-gray-800/40 hover:bg-gray-800 border border-gray-800 hover:border-gray-700 rounded-xl p-3 flex items-center justify-between gap-2 transition-all cursor-pointer group">
            ...
            <div class="flex items-center gap-2 shrink-0">
              <span class="w-2 h-2 rounded-full shadow-lg" :class="{
              ...
              class="text-gray-600 hover:text-red-400 p-1 rounded hover:bg-gray-700/50 transition-all opacity-0 group-hover:opacity-100" title="Kaydı Sil">🗑️</button>
            </div>
          </div>

          <div v-if="savedLocations?.length === 0" class="text-[11px] text-gray-600 text-center py-8 border border-dashed border-gray-800 rounded-xl">
